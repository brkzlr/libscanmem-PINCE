...
bool sm_execcommand(globals_t* vars, const char* commandline)
{
	...
	for (argc = 0; tok; ++argc) {
		...
		/* strtok expects NULL on subsequent calls */
		char* funcArg = argc == 0 ? dupstr : NULL;

		/* insert next token */
		argv[argc] = tok = strtok(funcArg, " \t");
	}
