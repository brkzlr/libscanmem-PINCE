...
void valtostr(const value_t* val, char* str, size_t n)
{
	...
	// clang-format off: clang-format makes this very ugly for some reason
#define FLAG_MACRO(bytes, string) \
	...
	if (val->flags & flag_u64b)
		np = snprintf(str, n, "%" PRIu64 ", %s", get_u64b(val), buf);
	else if (val->flags & flag_s64b)
		np = snprintf(str, n, "%" PRId64 ", %s", get_s64b(val), buf);
	else if (val->flags & flag_u32b)
		np = snprintf(str, n, "%" PRIu32 ", %s", get_u32b(val), buf);
	else if (val->flags & flag_s32b)
		np = snprintf(str, n, "%" PRId32 ", %s", get_s32b(val), buf);
	else if (val->flags & flag_u16b)
		np = snprintf(str, n, "%" PRIu16 ", %s", get_u16b(val), buf);
	else if (val->flags & flag_s16b)
		np = snprintf(str, n, "%" PRId16 ", %s", get_s16b(val), buf);
	else if (val->flags & flag_u8b)
		np = snprintf(str, n, "%" PRIu8 ", %s", get_u8b(val), buf);
	else if (val->flags & flag_s8b)
		np = snprintf(str, n, "%" PRId8 ", %s", get_s8b(val), buf);
	else if (val->flags & flag_f64b)
		np = snprintf(str, n, "%lg, %s", get_f64b(val), buf);
	else if (val->flags & flag_f32b)
		np = snprintf(str, n, "%g, %s", get_f32b(val), buf);
	else {
...
void uservalue2value(value_t* dst, const uservalue_t* src)
{
	...
	if (dst->flags & flag_f64b)
		set_f64b(dst, get_f64b(src));
	else if (dst->flags & flag_u64b)
		set_u64b(dst, get_u64b(src));
	else if (dst->flags & flag_s64b)
		set_s64b(dst, get_s64b(src));

	else if (dst->flags & flag_f32b)
		set_f32b(dst, get_f32b(src));
	else if (dst->flags & flag_u32b)
		set_u32b(dst, get_u32b(src));
	else if (dst->flags & flag_s32b)
		set_s32b(dst, get_s32b(src));

	else if (dst->flags & flag_u16b)
		set_u16b(dst, get_u16b(src));
	else if (dst->flags & flag_s16b)
		set_s16b(dst, get_s16b(src));

	else if (dst->flags & flag_u8b)
		set_u8b(dst, get_u8b(src));
	else if (dst->flags & flag_s8b)
		set_s8b(dst, get_s8b(src));
...
bool parse_uservalue_int(const char* nptr, uservalue_t* val)
{
	...
	if (valid_uint && unum <= UINT8_MAX) {
		val->flags |= flag_u8b;
		set_u8b(val, (uint8_t)unum);
	}
	if (valid_sint && snum >= INT8_MIN && snum <= INT8_MAX) {
		val->flags |= flag_s8b;
		set_s8b(val, (int8_t)snum);
	}
	if (valid_uint && unum <= UINT16_MAX) {
		val->flags |= flag_u16b;
		set_u16b(val, (uint16_t)unum);
	}
	if (valid_sint && snum >= INT16_MIN && snum <= INT16_MAX) {
		val->flags |= flag_s16b;
		set_s16b(val, (int16_t)snum);
	}
	if (valid_uint && unum <= UINT32_MAX) {
		val->flags |= flag_u32b;
		set_u32b(val, (uint32_t)unum);
	}
	if (valid_sint && snum >= INT32_MIN && snum <= INT32_MAX) {
		val->flags |= flag_s32b;
		set_s32b(val, (int32_t)snum);
	}
	if (valid_uint && unum <= UINT64_MAX) {
		val->flags |= flag_u64b;
		set_u64b(val, (uint64_t)unum);
	}
	if (valid_sint && snum >= INT64_MIN && snum <= INT64_MAX) {
		val->flags |= flag_s64b;
		set_s64b(val, (int64_t)snum);
	}
