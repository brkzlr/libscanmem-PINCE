...
bool handler__set(globals_t* vars, char** argv, unsigned argc)
{
	...
	if (argc < 2) {
		show_error("expected an argument.\n");
		return false;
	}
	...
	for (block = 0; block < argc - 1; ++block) {
		...
		if ((settings[block].value = strchr(argv[block + 1], '=')) == NULL) {
			...
			settings[block].matchids = strndup(argv[block + 1],
		...
		if ((delay = strchr(settings[block].value, '/')) != NULL) {
			...
			settings[block].value = strndup(settings[block].value,
		...
		for (block = 0; block < argc - 1; ++block) {
			...
			if (settings[block].matchids != NULL) {
				...
				if (parse_uintset(settings[block].matchids, &match_set, vars->num_matches)
				    == false) {
					show_error("failed to parse the set.\n");
					goto exit;
				}
...
exit:
	...
	for (block = 0; block < argc - 1; ++block) {
		free(settings[block].matchids);
		free(settings[block].value);
	}
	return ret;
}
	...
	while (reading_swath_index->first_byte_in_child) {
		...
		if (flags != flags_empty) {
			...
			while (np) {
				...
				if (address_ul < region_start + region->size && address_ul >= region_start) {
					region_id = region->id;
...
bool handler__delete(globals_t* vars, char** argv, unsigned argc)
{
	...
	if (argc != 2) {
		show_error("was expecting one argument.\n");
		return false;
	}
...
bool handler__dregion(globals_t* vars, char** argv, unsigned argc)
{
	...
	if (vars->target == 0) {
		show_error("no target specified.\n");
		return false;
	}
...
bool handler__lregions(globals_t* vars, char** argv, unsigned argc)
{
	...
	if (vars->target == 0) {
		show_error("no target has been specified.\n");
		return false;
	}
...
bool handler__operators(globals_t* vars, char** argv, unsigned argc)
{
	...
	if (argc == 1) {
		...
		show_error("too many values specified.");
		return false;
	}
	else {
		if (!parse_uservalue_number(argv[1], &val)) {
			show_error("bad value specified.");
			return false;
		}
	}
	...
	if (vars->matches != NULL) {
		 ...
		 *   = N, != N, < N, > N
		 */
		if (m == MATCHNOTCHANGED || m == MATCHCHANGED || m == MATCHDECREASED || m == MATCHINCREASED || m == MATCHDECREASEDBY || m == MATCHINCREASEDBY) {
...
bool handler__string(globals_t* vars, char** argv, unsigned argc)
{
	...
	if (vars->options.scan_data_type != STRING) {
		show_error("scan_data_type is not string.\n");
		return false;
	}
...
bool handler__shell(globals_t* vars, char** argv, unsigned argc)
{
	...
	if (argc < 2) {
		show_error("shell command requires an argument.\n");
		return false;
	}
...
bool handler__watch(globals_t* vars, char** argv, unsigned argc)
{
	...
	if (argv[1][0] == '\0' || *end != '\0') {
		show_error("sorry, couldn't parse `%s`\n",
		    argv[1]);
		return false;
	}
	...
	if (!loc.swath) {
		...
		show_info("use \"list\" to list matches.\n");
		return false;
	}
...
static inline scan_data_type_t parse_scan_data_type(const char* str)
{
	...
	if ((strcasecmp(str, "number") == 0) || (strcasecmp(str, "anynumber") == 0))
		return ANYNUMBER;
	if ((strcasecmp(str, "int") == 0) || (strcasecmp(str, "anyint") == 0) || (strcasecmp(str, "integer") == 0) || (strcasecmp(str, "anyinteger") == 0))
		return ANYINTEGER;
	if ((strcasecmp(str, "float") == 0) || (strcasecmp(str, "anyfloat") == 0))
		return ANYFLOAT;

	/* Ints */
	if ((strcasecmp(str, "i8") == 0) || (strcasecmp(str, "int8") == 0) || (strcasecmp(str, "integer8") == 0))
		return INTEGER8;
	if ((strcasecmp(str, "i16") == 0) || (strcasecmp(str, "int16") == 0) || (strcasecmp(str, "integer16") == 0))
		return INTEGER16;
	if ((strcasecmp(str, "i32") == 0) || (strcasecmp(str, "int32") == 0) || (strcasecmp(str, "integer32") == 0))
		return INTEGER32;
	if ((strcasecmp(str, "i64") == 0) || (strcasecmp(str, "int64") == 0) || (strcasecmp(str, "integer64") == 0))
		return INTEGER64;

	/* Floats */
	if ((strcasecmp(str, "f32") == 0) || (strcasecmp(str, "float32") == 0))
		return FLOAT32;
	if ((strcasecmp(str, "f64") == 0) || (strcasecmp(str, "float64") == 0) || (strcasecmp(str, "double") == 0))
		return FLOAT64;
...
bool handler__option(globals_t* vars, char** argv, unsigned argc)
{
	...
	if (strcasecmp(argv[1], "scan_data_type") == 0) {
		...
		if (st != (scan_data_type_t)(-1)) {
			...
			show_error("bad value for scan_data_type.\n");
			return false;
		}
	}
		...
		if (strcmp(argv[2], "1") == 0) {
			...
			vars->options.region_scan_level = REGION_ALL_RW;
		}
		else if (strcmp(argv[2], "4") == 0) {
			vars->options.region_scan_level = REGION_ALL;
		}
		else {
			show_error("bad value for region_scan_level.\n");
			return false;
		}
	}
		...
		if (strcmp(argv[2], "0") == 0) {
			...
			show_error("bad value for dump_with_ascii.\n");
			return false;
		}
	}
		...
		// data is big endian: swap if host is little endian
			...
			show_error("bad value for endianness.\n");
			return false;
		}
	}
	else {
