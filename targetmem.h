    ...
    matches_and_old_values_swath** swath_pointer_to_correct)
{
	...
	if (bytes_needed <= array->bytes_allocated) {
		...
		if (swath_pointer_to_correct) {
			(*swath_pointer_to_correct) = (matches_and_old_values_swath*)(((void*)(*swath_pointer_to_correct)) + ((void*)array - (void*)original_location));
		}
    ...
    match_flags new_flags)
{
	if (swath->number_of_bytes == 0) {
		...
		*array = allocate_enough_to_reach(*array, (void*)swath + sizeof(matches_and_old_values_swath) + sizeof(old_value_and_match_info), &swath);
		...
		size_t local_index_excess = remote_address - remote_address_of_last_element(swath);

		size_t local_address_excess = local_index_excess * sizeof(old_value_and_match_info);

		size_t needed_size_for_a_new_swath = sizeof(matches_and_old_values_swath) + sizeof(old_value_and_match_info);
		...
		if (local_address_excess >= needed_size_for_a_new_swath) {
			...
			*array = allocate_enough_to_reach(*array,
			    local_address_beyond_last_element(swath) + needed_size_for_a_new_swath, &swath);
			...
			*array = allocate_enough_to_reach(*array,
			    local_address_beyond_last_element(swath) + local_address_excess, &swath);
